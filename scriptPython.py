...
def System_apt():
    opc = input(Util.Title(txt='Opciones aptitude') + '\n'
        '1. Actualizar\n'
        '2. Limpiar\n'
        'Elige una opción: ') 
    if opc == '1':
...
def App_essential(txt=''):
    app_est = (Util.Title(txt = 'Programas Necesarios')  + '\n'
        f"{apt(txt='install')} "
...
def App_dependence(txt = ''):
    app_dpc = (Util.Title(txt='Algunas Dependencias') + '\n'
...
def App_desktop(txt=''):
    app_xfce4 = (Util.Title(txt = 'Programas Xfce4')  + '\n' +
        apt(txt='install') + ' '
    ...
    app_KDEplasma = (Util.Title(txt = 'Programas')  + '\n' +
        apt(txt='install') + ' '
        f'rofi {txt}')
    opc = input(Util.Title(txt='Programas para Escritorios') + '\n'
        '1. Xfce4\n'
        '2. Kdenlive\n'
        'Elige una opción: ')
    if opc == '1':
...
def App_optional(txt=''):
    flatpak = (Util.Title(txt = 'Flatpak')  + '\n'
    ...
    woeusb_ng = (Util.Title(txt = 'WoeUSB-ng')  + '\n'
        f"{apt(txt='install')} "
        'git p7zip-full python3-pip python3-wxgtk4.0 grub2-common grub-pc-bin '
        '&& sudo pip3 install WoeUSB-ng ')

    opc= input(Util.Title(txt='Aplicaciones opcionales') + '\n'
...
def Repository(txt=''):
    cfg = (f"{Util.Title(txt='Repositorios')}\n"
